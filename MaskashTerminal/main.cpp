        ...
        ret = "curl -X POST -s " + this->rpcEndpoint + \
            ...
            if(cmdPair.first == "eth_sendTransaction"){
                ...
                if(fp2 == NULL){
                    ...
                    const char *transactionSkeleton = cmdPair.second.data();//guess the characters of transaction are no more than 500 words
                    fputs(transactionSkeleton,fp2);
                    delete(transactionSkeleton);
                    //fputs(boost::lexical_cast<char*>(cmdPair.second),fp2);
                    fclose(fp2);
                }
            }
        return ret;
...
void netWork(char *ip_address){
   ...
   // std::cin>>interchargeServer;
    remoteEndpoint iServerEp = remoteEndpoint(boost::lexical_cast<string>(*ip_address));
    ...
    while(1){
        ...
        if(fp2 == NULL){
            ...
            fgets(v,255,fp2);
            tmp = boost::lexical_cast<string>(v);
            memset(v,'\0',255);
            sock.send_to(buffer(tmp),send_ep);
            tmp = '\0';
            fclose(fp2);
